...
public interface SinglePredicateIntThrow<E extends Exception> extends SinglePredicateInt {
    ...
    /**
     * 默认返回 true 的 SinglePredicateIntThrow 实例。
     */
    SinglePredicateIntThrow<?> DEFAULT_TRUE = (v1) -> BasicConstant.BOOLEAN_TRUE;

    /**
     * 默认返回 false 的 SinglePredicateIntThrow 实例。
     */
    SinglePredicateIntThrow<?> DEFAULT_FALSE = (v1) -> BasicConstant.BOOLEAN_FALSE;

    /**
     * 根据给定的布尔值返回对应的默认 SinglePredicateIntThrow 实例。
     *
     * @param value 给定的布尔值。
     * @return 如果 value 为 true，返回 DEFAULT_TRUE；否则返回 DEFAULT_FALSE。
     */
    @SuppressWarnings("unchecked")
    static <E extends Exception> SinglePredicateIntThrow<E> constant(boolean value) {
        return (SinglePredicateIntThrow<E>) (value ? DEFAULT_TRUE : DEFAULT_FALSE);
    }
