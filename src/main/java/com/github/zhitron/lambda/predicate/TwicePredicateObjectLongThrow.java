...
public interface TwicePredicateObjectLongThrow<T, E extends Exception> extends TwicePredicateObjectLong<T> {
    ...
    /**
     * 默认返回 true 的 TwicePredicateObjectLongThrow 实例。
     */
    TwicePredicateObjectLongThrow<?, ?> DEFAULT_TRUE = (v1, v2) -> BasicConstant.BOOLEAN_TRUE;

    /**
     * 默认返回 false 的 TwicePredicateObjectLongThrow 实例。
     */
    TwicePredicateObjectLongThrow<?, ?> DEFAULT_FALSE = (v1, v2) -> BasicConstant.BOOLEAN_FALSE;

    /**
     * 根据给定的布尔值返回对应的默认 TwicePredicateObjectLongThrow 实例。
     *
     * @param value 给定的布尔值。
     * @return 如果 value 为 true，返回 DEFAULT_TRUE；否则返回 DEFAULT_FALSE。
     */
    @SuppressWarnings("unchecked")
    static <T, E extends Exception> TwicePredicateObjectLongThrow<T, E> constant(boolean value) {
        return (TwicePredicateObjectLongThrow<T, E>) (value ? DEFAULT_TRUE : DEFAULT_FALSE);
    }
