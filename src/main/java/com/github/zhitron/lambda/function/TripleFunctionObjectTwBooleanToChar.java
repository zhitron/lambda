 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectTwBooleanToChar<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.CHAR_ZERO 值。
     */
    TripleFunctionObjectTwBooleanToChar<?> EMPTY = (v1, v2, v3) -> BasicConstant.CHAR_ZERO;
    ...
    static <T> TripleFunctionObjectTwBooleanToChar<T> empty() {
        return (TripleFunctionObjectTwBooleanToChar<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectTwBooleanToChar<T> constant(char value) {
     ...
     */
    char apply(T v1, boolean v2, boolean v3);
}
