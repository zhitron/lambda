 ...
 * 该接口扩展自 {@link TripleFunctionIntToChar}，增加了异常处理能力。
 *
 * @param <E> 异常类型，必须是 {@link Exception} 的子类
 * @author zhitron
 */
@FunctionalInterface
public interface TripleFunctionIntToCharThrow<E extends Exception> extends TripleFunctionIntToChar {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.CHAR_ZERO 值。
     */
    TripleFunctionIntToCharThrow<?> EMPTY = (v1, v2, v3) -> BasicConstant.CHAR_ZERO;
    ...
    static <E extends Exception> TripleFunctionIntToCharThrow<E> empty() {
        return (TripleFunctionIntToCharThrow<E>) EMPTY;
    }
     ...
     */
    static <E extends Exception> TripleFunctionIntToCharThrow<E> constant(char value) {
     ...
     * @throws E 抛出执行过程中的异常
     */
    char applyThrow(int v1, int v2, int v3) throws E;
     ...
     */
    @Override
    default char apply(int v1, int v2, int v3) {
        try {
