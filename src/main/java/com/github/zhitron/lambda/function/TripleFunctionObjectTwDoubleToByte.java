 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectTwDoubleToByte<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.BYTE_ZERO 值。
     */
    TripleFunctionObjectTwDoubleToByte<?> EMPTY = (v1, v2, v3) -> BasicConstant.BYTE_ZERO;
    ...
    static <T> TripleFunctionObjectTwDoubleToByte<T> empty() {
        return (TripleFunctionObjectTwDoubleToByte<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectTwDoubleToByte<T> constant(byte value) {
     ...
     */
    byte apply(T v1, double v2, double v3);
}
