 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectIntLongToByte<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.BYTE_ZERO 值。
     */
    TripleFunctionObjectIntLongToByte<?> EMPTY = (v1, v2, v3) -> BasicConstant.BYTE_ZERO;
    ...
    static <T> TripleFunctionObjectIntLongToByte<T> empty() {
        return (TripleFunctionObjectIntLongToByte<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectIntLongToByte<T> constant(byte value) {
     ...
     */
    byte apply(T v1, int v2, long v3);
}
