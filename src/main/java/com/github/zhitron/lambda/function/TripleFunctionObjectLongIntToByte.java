 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectLongIntToByte<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.BYTE_ZERO 值。
     */
    TripleFunctionObjectLongIntToByte<?> EMPTY = (v1, v2, v3) -> BasicConstant.BYTE_ZERO;
    ...
    static <T> TripleFunctionObjectLongIntToByte<T> empty() {
        return (TripleFunctionObjectLongIntToByte<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectLongIntToByte<T> constant(byte value) {
     ...
     */
    byte apply(T v1, long v2, int v3);
}
