 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectTwDoubleToInt<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.INT_ZERO 值。
     */
    TripleFunctionObjectTwDoubleToInt<?> EMPTY = (v1, v2, v3) -> BasicConstant.INT_ZERO;
    ...
    static <T> TripleFunctionObjectTwDoubleToInt<T> empty() {
        return (TripleFunctionObjectTwDoubleToInt<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectTwDoubleToInt<T> constant(int value) {
     ...
     */
    int apply(T v1, double v2, double v3);
}
