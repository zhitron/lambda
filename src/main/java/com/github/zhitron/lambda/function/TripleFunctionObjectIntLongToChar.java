 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectIntLongToChar<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.CHAR_ZERO 值。
     */
    TripleFunctionObjectIntLongToChar<?> EMPTY = (v1, v2, v3) -> BasicConstant.CHAR_ZERO;
    ...
    static <T> TripleFunctionObjectIntLongToChar<T> empty() {
        return (TripleFunctionObjectIntLongToChar<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectIntLongToChar<T> constant(char value) {
     ...
     */
    char apply(T v1, int v2, long v3);
}
