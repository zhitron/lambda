 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectTwLongToByte<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.BYTE_ZERO 值。
     */
    TripleFunctionObjectTwLongToByte<?> EMPTY = (v1, v2, v3) -> BasicConstant.BYTE_ZERO;
    ...
    static <T> TripleFunctionObjectTwLongToByte<T> empty() {
        return (TripleFunctionObjectTwLongToByte<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectTwLongToByte<T> constant(byte value) {
     ...
     */
    byte apply(T v1, long v2, long v3);
}
