 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectTwBooleanToLong<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.LONG_ZERO 值。
     */
    TripleFunctionObjectTwBooleanToLong<?> EMPTY = (v1, v2, v3) -> BasicConstant.LONG_ZERO;
    ...
    static <T> TripleFunctionObjectTwBooleanToLong<T> empty() {
        return (TripleFunctionObjectTwBooleanToLong<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectTwBooleanToLong<T> constant(long value) {
     ...
     */
    long apply(T v1, boolean v2, boolean v3);
}
