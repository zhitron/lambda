 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectIntDoubleToLong<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.LONG_ZERO 值。
     */
    TripleFunctionObjectIntDoubleToLong<?> EMPTY = (v1, v2, v3) -> BasicConstant.LONG_ZERO;
    ...
    static <T> TripleFunctionObjectIntDoubleToLong<T> empty() {
        return (TripleFunctionObjectIntDoubleToLong<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectIntDoubleToLong<T> constant(long value) {
     ...
     */
    long apply(T v1, int v2, double v3);
}
