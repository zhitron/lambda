 ...
 */
@FunctionalInterface
public interface SingleFunctionObjectToByte<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.BYTE_ZERO 值。
     */
    SingleFunctionObjectToByte<?> EMPTY = (v1) -> BasicConstant.BYTE_ZERO;
    ...
    static <T> SingleFunctionObjectToByte<T> empty() {
        return (SingleFunctionObjectToByte<T>) EMPTY;
    }
     ...
     */
    static <T> SingleFunctionObjectToByte<T> constant(byte value) {
        return (v1) -> value;
    }
