 ...
 * 该接口扩展自 {@link QuadrupleFunctionIntToShort}，增加了异常处理能力。
 *
 * @param <E> 异常类型，必须是 {@link Exception} 的子类
 * @author zhitron
 */
@FunctionalInterface
public interface QuadrupleFunctionIntToShortThrow<E extends Exception> extends QuadrupleFunctionIntToShort {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.SHORT_ZERO 值。
     */
    QuadrupleFunctionIntToShortThrow<?> EMPTY = (v1, v2, v3, v4) -> BasicConstant.SHORT_ZERO;
    ...
    static <E extends Exception> QuadrupleFunctionIntToShortThrow<E> empty() {
        return (QuadrupleFunctionIntToShortThrow<E>) EMPTY;
    }
     ...
     */
    static <E extends Exception> QuadrupleFunctionIntToShortThrow<E> constant(short value) {
     ...
     * @throws E 抛出执行过程中的异常
     */
    short applyThrow(int v1, int v2, int v3, int v4) throws E;
     ...
     */
    @Override
    default short apply(int v1, int v2, int v3, int v4) {
        try {
