 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectTwBooleanToByte<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.BYTE_ZERO 值。
     */
    TripleFunctionObjectTwBooleanToByte<?> EMPTY = (v1, v2, v3) -> BasicConstant.BYTE_ZERO;
    ...
    static <T> TripleFunctionObjectTwBooleanToByte<T> empty() {
        return (TripleFunctionObjectTwBooleanToByte<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectTwBooleanToByte<T> constant(byte value) {
     ...
     */
    byte apply(T v1, boolean v2, boolean v3);
}
