 ...
 * 该接口扩展自 {@link QuadrupleFunctionIntToByte}，增加了异常处理能力。
 *
 * @param <E> 异常类型，必须是 {@link Exception} 的子类
 * @author zhitron
 */
@FunctionalInterface
public interface QuadrupleFunctionIntToByteThrow<E extends Exception> extends QuadrupleFunctionIntToByte {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.BYTE_ZERO 值。
     */
    QuadrupleFunctionIntToByteThrow<?> EMPTY = (v1, v2, v3, v4) -> BasicConstant.BYTE_ZERO;
    ...
    static <E extends Exception> QuadrupleFunctionIntToByteThrow<E> empty() {
        return (QuadrupleFunctionIntToByteThrow<E>) EMPTY;
    }
     ...
     */
    static <E extends Exception> QuadrupleFunctionIntToByteThrow<E> constant(byte value) {
     ...
     * @throws E 抛出执行过程中的异常
     */
    byte applyThrow(int v1, int v2, int v3, int v4) throws E;
     ...
     */
    @Override
    default byte apply(int v1, int v2, int v3, int v4) {
        try {
