 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectDoubleIntToChar<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.CHAR_ZERO 值。
     */
    TripleFunctionObjectDoubleIntToChar<?> EMPTY = (v1, v2, v3) -> BasicConstant.CHAR_ZERO;
    ...
    static <T> TripleFunctionObjectDoubleIntToChar<T> empty() {
        return (TripleFunctionObjectDoubleIntToChar<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectDoubleIntToChar<T> constant(char value) {
     ...
     */
    char apply(T v1, double v2, int v3);
}
