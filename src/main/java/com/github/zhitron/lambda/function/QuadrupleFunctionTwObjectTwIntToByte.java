 ...
 */
@FunctionalInterface
public interface QuadrupleFunctionTwObjectTwIntToByte<T, U> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.BYTE_ZERO 值。
     */
    QuadrupleFunctionTwObjectTwIntToByte<?, ?> EMPTY = (v1, v2, v3, v4) -> BasicConstant.BYTE_ZERO;
    ...
    static <T, U> QuadrupleFunctionTwObjectTwIntToByte<T, U> empty() {
        return (QuadrupleFunctionTwObjectTwIntToByte<T, U>) EMPTY;
    }
     ...
     */
    static <T, U> QuadrupleFunctionTwObjectTwIntToByte<T, U> constant(byte value) {
     ...
     */
    byte apply(T v1, U v2, int v3, int v4);
}
