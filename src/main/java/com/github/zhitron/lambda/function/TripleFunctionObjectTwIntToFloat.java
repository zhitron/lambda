 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectTwIntToFloat<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.FLOAT_ZERO 值。
     */
    TripleFunctionObjectTwIntToFloat<?> EMPTY = (v1, v2, v3) -> BasicConstant.FLOAT_ZERO;
    ...
    static <T> TripleFunctionObjectTwIntToFloat<T> empty() {
        return (TripleFunctionObjectTwIntToFloat<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectTwIntToFloat<T> constant(float value) {
     ...
     */
    float apply(T v1, int v2, int v3);
}
