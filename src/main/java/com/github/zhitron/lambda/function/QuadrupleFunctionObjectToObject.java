 ...
 */
@FunctionalInterface
public interface QuadrupleFunctionObjectToObject<T, U, V, O, R> {

    /**
     * 一个空实现的实例，它总是返回 null 值。
     */
    QuadrupleFunctionObjectToObject<?, ?, ?, ?, ?> EMPTY = (v1, v2, v3, v4) -> null;
    ...
    static <T, U, V, O, R> QuadrupleFunctionObjectToObject<T, U, V, O, R> empty() {
        return (QuadrupleFunctionObjectToObject<T, U, V, O, R>) EMPTY;
    }
     ...
     */
    static <T, U, V, O, R> QuadrupleFunctionObjectToObject<T, U, V, O, R> constant(R value) {
