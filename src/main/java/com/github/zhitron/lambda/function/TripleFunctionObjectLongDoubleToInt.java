 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectLongDoubleToInt<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.INT_ZERO 值。
     */
    TripleFunctionObjectLongDoubleToInt<?> EMPTY = (v1, v2, v3) -> BasicConstant.INT_ZERO;
    ...
    static <T> TripleFunctionObjectLongDoubleToInt<T> empty() {
        return (TripleFunctionObjectLongDoubleToInt<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectLongDoubleToInt<T> constant(int value) {
     ...
     */
    int apply(T v1, long v2, double v3);
}
