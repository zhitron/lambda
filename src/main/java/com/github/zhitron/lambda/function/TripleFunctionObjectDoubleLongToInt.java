 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectDoubleLongToInt<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.INT_ZERO 值。
     */
    TripleFunctionObjectDoubleLongToInt<?> EMPTY = (v1, v2, v3) -> BasicConstant.INT_ZERO;
    ...
    static <T> TripleFunctionObjectDoubleLongToInt<T> empty() {
        return (TripleFunctionObjectDoubleLongToInt<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectDoubleLongToInt<T> constant(int value) {
     ...
     */
    int apply(T v1, double v2, long v3);
}
