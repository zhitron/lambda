 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectTwDoubleToChar<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.CHAR_ZERO 值。
     */
    TripleFunctionObjectTwDoubleToChar<?> EMPTY = (v1, v2, v3) -> BasicConstant.CHAR_ZERO;
    ...
    static <T> TripleFunctionObjectTwDoubleToChar<T> empty() {
        return (TripleFunctionObjectTwDoubleToChar<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectTwDoubleToChar<T> constant(char value) {
     ...
     */
    char apply(T v1, double v2, double v3);
}
