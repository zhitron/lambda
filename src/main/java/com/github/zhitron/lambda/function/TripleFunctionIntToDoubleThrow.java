 ...
 * 该接口扩展自 {@link TripleFunctionIntToDouble}，增加了异常处理能力。
 *
 * @param <E> 异常类型，必须是 {@link Exception} 的子类
 * @author zhitron
 */
@FunctionalInterface
public interface TripleFunctionIntToDoubleThrow<E extends Exception> extends TripleFunctionIntToDouble {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.DOUBLE_ZERO 值。
     */
    TripleFunctionIntToDoubleThrow<?> EMPTY = (v1, v2, v3) -> BasicConstant.DOUBLE_ZERO;
    ...
    static <E extends Exception> TripleFunctionIntToDoubleThrow<E> empty() {
        return (TripleFunctionIntToDoubleThrow<E>) EMPTY;
    }
     ...
     */
    static <E extends Exception> TripleFunctionIntToDoubleThrow<E> constant(double value) {
     ...
     * @throws E 抛出执行过程中的异常
     */
    double applyThrow(int v1, int v2, int v3) throws E;
     ...
     */
    @Override
    default double apply(int v1, int v2, int v3) {
        try {
