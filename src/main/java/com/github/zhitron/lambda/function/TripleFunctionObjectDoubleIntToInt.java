 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectDoubleIntToInt<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.INT_ZERO 值。
     */
    TripleFunctionObjectDoubleIntToInt<?> EMPTY = (v1, v2, v3) -> BasicConstant.INT_ZERO;
    ...
    static <T> TripleFunctionObjectDoubleIntToInt<T> empty() {
        return (TripleFunctionObjectDoubleIntToInt<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectDoubleIntToInt<T> constant(int value) {
     ...
     */
    int apply(T v1, double v2, int v3);
}
