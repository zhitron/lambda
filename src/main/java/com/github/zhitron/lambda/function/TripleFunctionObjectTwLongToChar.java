 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectTwLongToChar<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.CHAR_ZERO 值。
     */
    TripleFunctionObjectTwLongToChar<?> EMPTY = (v1, v2, v3) -> BasicConstant.CHAR_ZERO;
    ...
    static <T> TripleFunctionObjectTwLongToChar<T> empty() {
        return (TripleFunctionObjectTwLongToChar<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectTwLongToChar<T> constant(char value) {
     ...
     */
    char apply(T v1, long v2, long v3);
}
