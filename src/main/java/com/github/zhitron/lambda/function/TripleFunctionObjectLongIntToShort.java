 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectLongIntToShort<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.SHORT_ZERO 值。
     */
    TripleFunctionObjectLongIntToShort<?> EMPTY = (v1, v2, v3) -> BasicConstant.SHORT_ZERO;
    ...
    static <T> TripleFunctionObjectLongIntToShort<T> empty() {
        return (TripleFunctionObjectLongIntToShort<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectLongIntToShort<T> constant(short value) {
     ...
     */
    short apply(T v1, long v2, int v3);
}
