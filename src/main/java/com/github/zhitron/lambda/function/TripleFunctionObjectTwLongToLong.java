 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectTwLongToLong<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.LONG_ZERO 值。
     */
    TripleFunctionObjectTwLongToLong<?> EMPTY = (v1, v2, v3) -> BasicConstant.LONG_ZERO;
    ...
    static <T> TripleFunctionObjectTwLongToLong<T> empty() {
        return (TripleFunctionObjectTwLongToLong<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectTwLongToLong<T> constant(long value) {
     ...
     */
    long apply(T v1, long v2, long v3);
}
