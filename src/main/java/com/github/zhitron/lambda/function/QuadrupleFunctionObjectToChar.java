 ...
 */
@FunctionalInterface
public interface QuadrupleFunctionObjectToChar<T, U, V, O> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.CHAR_ZERO 值。
     */
    QuadrupleFunctionObjectToChar<?, ?, ?, ?> EMPTY = (v1, v2, v3, v4) -> BasicConstant.CHAR_ZERO;
    ...
    static <T, U, V, O> QuadrupleFunctionObjectToChar<T, U, V, O> empty() {
        return (QuadrupleFunctionObjectToChar<T, U, V, O>) EMPTY;
    }
     ...
     */
    static <T, U, V, O> QuadrupleFunctionObjectToChar<T, U, V, O> constant(char value) {
     ...
     */
    char apply(T v1, U v2, V v3, O v4);
}
