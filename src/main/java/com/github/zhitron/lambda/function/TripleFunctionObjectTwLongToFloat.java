 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectTwLongToFloat<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.FLOAT_ZERO 值。
     */
    TripleFunctionObjectTwLongToFloat<?> EMPTY = (v1, v2, v3) -> BasicConstant.FLOAT_ZERO;
    ...
    static <T> TripleFunctionObjectTwLongToFloat<T> empty() {
        return (TripleFunctionObjectTwLongToFloat<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectTwLongToFloat<T> constant(float value) {
     ...
     */
    float apply(T v1, long v2, long v3);
}
