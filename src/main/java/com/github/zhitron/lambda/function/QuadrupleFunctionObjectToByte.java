 ...
 */
@FunctionalInterface
public interface QuadrupleFunctionObjectToByte<T, U, V, O> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.BYTE_ZERO 值。
     */
    QuadrupleFunctionObjectToByte<?, ?, ?, ?> EMPTY = (v1, v2, v3, v4) -> BasicConstant.BYTE_ZERO;
    ...
    static <T, U, V, O> QuadrupleFunctionObjectToByte<T, U, V, O> empty() {
        return (QuadrupleFunctionObjectToByte<T, U, V, O>) EMPTY;
    }
     ...
     */
    static <T, U, V, O> QuadrupleFunctionObjectToByte<T, U, V, O> constant(byte value) {
     ...
     */
    byte apply(T v1, U v2, V v3, O v4);
}
