 ...
 * 该接口扩展自 {@link QuadrupleFunctionIntToFloat}，增加了异常处理能力。
 *
 * @param <E> 异常类型，必须是 {@link Exception} 的子类
 * @author zhitron
 */
@FunctionalInterface
public interface QuadrupleFunctionIntToFloatThrow<E extends Exception> extends QuadrupleFunctionIntToFloat {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.FLOAT_ZERO 值。
     */
    QuadrupleFunctionIntToFloatThrow<?> EMPTY = (v1, v2, v3, v4) -> BasicConstant.FLOAT_ZERO;
    ...
    static <E extends Exception> QuadrupleFunctionIntToFloatThrow<E> empty() {
        return (QuadrupleFunctionIntToFloatThrow<E>) EMPTY;
    }
     ...
     */
    static <E extends Exception> QuadrupleFunctionIntToFloatThrow<E> constant(float value) {
     ...
     * @throws E 抛出执行过程中的异常
     */
    float applyThrow(int v1, int v2, int v3, int v4) throws E;
     ...
     */
    @Override
    default float apply(int v1, int v2, int v3, int v4) {
        try {
