 ...
 * 该接口扩展自 {@link TripleFunctionIntToByte}，增加了异常处理能力。
 *
 * @param <E> 异常类型，必须是 {@link Exception} 的子类
 * @author zhitron
 */
@FunctionalInterface
public interface TripleFunctionIntToByteThrow<E extends Exception> extends TripleFunctionIntToByte {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.BYTE_ZERO 值。
     */
    TripleFunctionIntToByteThrow<?> EMPTY = (v1, v2, v3) -> BasicConstant.BYTE_ZERO;
    ...
    static <E extends Exception> TripleFunctionIntToByteThrow<E> empty() {
        return (TripleFunctionIntToByteThrow<E>) EMPTY;
    }
     ...
     */
    static <E extends Exception> TripleFunctionIntToByteThrow<E> constant(byte value) {
     ...
     * @throws E 抛出执行过程中的异常
     */
    byte applyThrow(int v1, int v2, int v3) throws E;
     ...
     */
    @Override
    default byte apply(int v1, int v2, int v3) {
        try {
