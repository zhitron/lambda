 ...
 */
@FunctionalInterface
public interface QuadrupleFunctionObjectToLong<T, U, V, O> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.LONG_ZERO 值。
     */
    QuadrupleFunctionObjectToLong<?, ?, ?, ?> EMPTY = (v1, v2, v3, v4) -> BasicConstant.LONG_ZERO;
    ...
    static <T, U, V, O> QuadrupleFunctionObjectToLong<T, U, V, O> empty() {
        return (QuadrupleFunctionObjectToLong<T, U, V, O>) EMPTY;
    }
     ...
     */
    static <T, U, V, O> QuadrupleFunctionObjectToLong<T, U, V, O> constant(long value) {
     ...
     */
    long apply(T v1, U v2, V v3, O v4);
}
