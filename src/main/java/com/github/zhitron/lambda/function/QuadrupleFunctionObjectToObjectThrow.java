 ...
 * 该接口扩展自 {@link QuadrupleFunctionObjectToObject}，增加了异常处理能力。
 *
 * @param <T> 第 1 个参数类型。
 * @param <U> 第 2 个参数类型。
 * @param <V> 第 3 个参数类型。
 ...
 */
@FunctionalInterface
public interface QuadrupleFunctionObjectToObjectThrow<T, U, V, O, R, E extends Exception> extends QuadrupleFunctionObjectToObject<T, U, V, O, R> {

    /**
     * 一个空实现的实例，它总是返回 null 值。
     */
    QuadrupleFunctionObjectToObjectThrow<?, ?, ?, ?, ?, ?> EMPTY = (v1, v2, v3, v4) -> null;
    ...
    static <T, U, V, O, R, E extends Exception> QuadrupleFunctionObjectToObjectThrow<T, U, V, O, R, E> empty() {
        return (QuadrupleFunctionObjectToObjectThrow<T, U, V, O, R, E>) EMPTY;
    }
     ...
     */
    static <T, U, V, O, R, E extends Exception> QuadrupleFunctionObjectToObjectThrow<T, U, V, O, R, E> constant(R value) {
