 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectDoubleIntToByte<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.BYTE_ZERO 值。
     */
    TripleFunctionObjectDoubleIntToByte<?> EMPTY = (v1, v2, v3) -> BasicConstant.BYTE_ZERO;
    ...
    static <T> TripleFunctionObjectDoubleIntToByte<T> empty() {
        return (TripleFunctionObjectDoubleIntToByte<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectDoubleIntToByte<T> constant(byte value) {
     ...
     */
    byte apply(T v1, double v2, int v3);
}
