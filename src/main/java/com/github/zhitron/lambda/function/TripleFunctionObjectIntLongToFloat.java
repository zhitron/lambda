 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectIntLongToFloat<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.FLOAT_ZERO 值。
     */
    TripleFunctionObjectIntLongToFloat<?> EMPTY = (v1, v2, v3) -> BasicConstant.FLOAT_ZERO;
    ...
    static <T> TripleFunctionObjectIntLongToFloat<T> empty() {
        return (TripleFunctionObjectIntLongToFloat<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectIntLongToFloat<T> constant(float value) {
     ...
     */
    float apply(T v1, int v2, long v3);
}
