 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectTwLongToInt<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.INT_ZERO 值。
     */
    TripleFunctionObjectTwLongToInt<?> EMPTY = (v1, v2, v3) -> BasicConstant.INT_ZERO;
    ...
    static <T> TripleFunctionObjectTwLongToInt<T> empty() {
        return (TripleFunctionObjectTwLongToInt<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectTwLongToInt<T> constant(int value) {
     ...
     */
    int apply(T v1, long v2, long v3);
}
