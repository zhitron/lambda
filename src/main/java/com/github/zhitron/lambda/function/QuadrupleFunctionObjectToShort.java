 ...
 */
@FunctionalInterface
public interface QuadrupleFunctionObjectToShort<T, U, V, O> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.SHORT_ZERO 值。
     */
    QuadrupleFunctionObjectToShort<?, ?, ?, ?> EMPTY = (v1, v2, v3, v4) -> BasicConstant.SHORT_ZERO;
    ...
    static <T, U, V, O> QuadrupleFunctionObjectToShort<T, U, V, O> empty() {
        return (QuadrupleFunctionObjectToShort<T, U, V, O>) EMPTY;
    }
     ...
     */
    static <T, U, V, O> QuadrupleFunctionObjectToShort<T, U, V, O> constant(short value) {
     ...
     */
    short apply(T v1, U v2, V v3, O v4);
}
