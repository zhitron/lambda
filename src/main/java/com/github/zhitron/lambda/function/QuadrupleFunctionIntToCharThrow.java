 ...
 * 该接口扩展自 {@link QuadrupleFunctionIntToChar}，增加了异常处理能力。
 *
 * @param <E> 异常类型，必须是 {@link Exception} 的子类
 * @author zhitron
 */
@FunctionalInterface
public interface QuadrupleFunctionIntToCharThrow<E extends Exception> extends QuadrupleFunctionIntToChar {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.CHAR_ZERO 值。
     */
    QuadrupleFunctionIntToCharThrow<?> EMPTY = (v1, v2, v3, v4) -> BasicConstant.CHAR_ZERO;
    ...
    static <E extends Exception> QuadrupleFunctionIntToCharThrow<E> empty() {
        return (QuadrupleFunctionIntToCharThrow<E>) EMPTY;
    }
     ...
     */
    static <E extends Exception> QuadrupleFunctionIntToCharThrow<E> constant(char value) {
     ...
     * @throws E 抛出执行过程中的异常
     */
    char applyThrow(int v1, int v2, int v3, int v4) throws E;
     ...
     */
    @Override
    default char apply(int v1, int v2, int v3, int v4) {
        try {
