 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectLongIntToFloat<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.FLOAT_ZERO 值。
     */
    TripleFunctionObjectLongIntToFloat<?> EMPTY = (v1, v2, v3) -> BasicConstant.FLOAT_ZERO;
    ...
    static <T> TripleFunctionObjectLongIntToFloat<T> empty() {
        return (TripleFunctionObjectLongIntToFloat<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectLongIntToFloat<T> constant(float value) {
     ...
     */
    float apply(T v1, long v2, int v3);
}
