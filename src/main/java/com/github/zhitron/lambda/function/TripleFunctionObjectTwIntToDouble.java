 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectTwIntToDouble<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.DOUBLE_ZERO 值。
     */
    TripleFunctionObjectTwIntToDouble<?> EMPTY = (v1, v2, v3) -> BasicConstant.DOUBLE_ZERO;
    ...
    static <T> TripleFunctionObjectTwIntToDouble<T> empty() {
        return (TripleFunctionObjectTwIntToDouble<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectTwIntToDouble<T> constant(double value) {
     ...
     */
    double apply(T v1, int v2, int v3);
}
