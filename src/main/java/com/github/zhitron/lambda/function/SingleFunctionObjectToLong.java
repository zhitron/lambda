 ...
 */
@FunctionalInterface
public interface SingleFunctionObjectToLong<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.LONG_ZERO 值。
     */
    SingleFunctionObjectToLong<?> EMPTY = (v1) -> BasicConstant.LONG_ZERO;
    ...
    static <T> SingleFunctionObjectToLong<T> empty() {
        return (SingleFunctionObjectToLong<T>) EMPTY;
    }
     ...
     */
    static <T> SingleFunctionObjectToLong<T> constant(long value) {
        return (v1) -> value;
    }
