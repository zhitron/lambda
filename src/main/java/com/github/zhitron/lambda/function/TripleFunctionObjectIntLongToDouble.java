 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectIntLongToDouble<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.DOUBLE_ZERO 值。
     */
    TripleFunctionObjectIntLongToDouble<?> EMPTY = (v1, v2, v3) -> BasicConstant.DOUBLE_ZERO;
    ...
    static <T> TripleFunctionObjectIntLongToDouble<T> empty() {
        return (TripleFunctionObjectIntLongToDouble<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectIntLongToDouble<T> constant(double value) {
     ...
     */
    double apply(T v1, int v2, long v3);
}
