 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectTwIntToByte<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.BYTE_ZERO 值。
     */
    TripleFunctionObjectTwIntToByte<?> EMPTY = (v1, v2, v3) -> BasicConstant.BYTE_ZERO;
    ...
    static <T> TripleFunctionObjectTwIntToByte<T> empty() {
        return (TripleFunctionObjectTwIntToByte<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectTwIntToByte<T> constant(byte value) {
