 ...
 * 该接口扩展自 {@link TripleFunctionIntToFloat}，增加了异常处理能力。
 *
 * @param <E> 异常类型，必须是 {@link Exception} 的子类
 * @author zhitron
 */
@FunctionalInterface
public interface TripleFunctionIntToFloatThrow<E extends Exception> extends TripleFunctionIntToFloat {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.FLOAT_ZERO 值。
     */
    TripleFunctionIntToFloatThrow<?> EMPTY = (v1, v2, v3) -> BasicConstant.FLOAT_ZERO;
    ...
    static <E extends Exception> TripleFunctionIntToFloatThrow<E> empty() {
        return (TripleFunctionIntToFloatThrow<E>) EMPTY;
    }
     ...
     */
    static <E extends Exception> TripleFunctionIntToFloatThrow<E> constant(float value) {
     ...
     * @throws E 抛出执行过程中的异常
     */
    float applyThrow(int v1, int v2, int v3) throws E;
     ...
     */
    @Override
    default float apply(int v1, int v2, int v3) {
        try {
