 ...
 * 该接口扩展自 {@link TripleFunctionIntToShort}，增加了异常处理能力。
 *
 * @param <E> 异常类型，必须是 {@link Exception} 的子类
 * @author zhitron
 */
@FunctionalInterface
public interface TripleFunctionIntToShortThrow<E extends Exception> extends TripleFunctionIntToShort {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.SHORT_ZERO 值。
     */
    TripleFunctionIntToShortThrow<?> EMPTY = (v1, v2, v3) -> BasicConstant.SHORT_ZERO;
    ...
    static <E extends Exception> TripleFunctionIntToShortThrow<E> empty() {
        return (TripleFunctionIntToShortThrow<E>) EMPTY;
    }
     ...
     */
    static <E extends Exception> TripleFunctionIntToShortThrow<E> constant(short value) {
     ...
     * @throws E 抛出执行过程中的异常
     */
    short applyThrow(int v1, int v2, int v3) throws E;
     ...
     */
    @Override
    default short apply(int v1, int v2, int v3) {
        try {
