 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectIntDoubleToChar<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.CHAR_ZERO 值。
     */
    TripleFunctionObjectIntDoubleToChar<?> EMPTY = (v1, v2, v3) -> BasicConstant.CHAR_ZERO;
    ...
    static <T> TripleFunctionObjectIntDoubleToChar<T> empty() {
        return (TripleFunctionObjectIntDoubleToChar<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectIntDoubleToChar<T> constant(char value) {
     ...
     */
    char apply(T v1, int v2, double v3);
}
