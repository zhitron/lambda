 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectDoubleIntToLong<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.LONG_ZERO 值。
     */
    TripleFunctionObjectDoubleIntToLong<?> EMPTY = (v1, v2, v3) -> BasicConstant.LONG_ZERO;
    ...
    static <T> TripleFunctionObjectDoubleIntToLong<T> empty() {
        return (TripleFunctionObjectDoubleIntToLong<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectDoubleIntToLong<T> constant(long value) {
     ...
     */
    long apply(T v1, double v2, int v3);
}
