 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectLongIntToDouble<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.DOUBLE_ZERO 值。
     */
    TripleFunctionObjectLongIntToDouble<?> EMPTY = (v1, v2, v3) -> BasicConstant.DOUBLE_ZERO;
    ...
    static <T> TripleFunctionObjectLongIntToDouble<T> empty() {
        return (TripleFunctionObjectLongIntToDouble<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectLongIntToDouble<T> constant(double value) {
     ...
     */
    double apply(T v1, long v2, int v3);
}
