 ...
 */
@FunctionalInterface
public interface QuadrupleFunctionObjectLongIntLongToInt<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.INT_ZERO 值。
     */
    QuadrupleFunctionObjectLongIntLongToInt<?> EMPTY = (v1, v2, v3, v4) -> BasicConstant.INT_ZERO;
    ...
    static <T> QuadrupleFunctionObjectLongIntLongToInt<T> empty() {
        return (QuadrupleFunctionObjectLongIntLongToInt<T>) EMPTY;
    }
     ...
     */
    static <T> QuadrupleFunctionObjectLongIntLongToInt<T> constant(int value) {
     ...
     */
    int apply(T v1, long v2, int v3, long v4);
}
