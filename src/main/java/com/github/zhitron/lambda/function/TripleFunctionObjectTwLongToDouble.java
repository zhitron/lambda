 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectTwLongToDouble<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.DOUBLE_ZERO 值。
     */
    TripleFunctionObjectTwLongToDouble<?> EMPTY = (v1, v2, v3) -> BasicConstant.DOUBLE_ZERO;
    ...
    static <T> TripleFunctionObjectTwLongToDouble<T> empty() {
        return (TripleFunctionObjectTwLongToDouble<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectTwLongToDouble<T> constant(double value) {
     ...
     */
    double apply(T v1, long v2, long v3);
}
