 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectLongIntToLong<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.LONG_ZERO 值。
     */
    TripleFunctionObjectLongIntToLong<?> EMPTY = (v1, v2, v3) -> BasicConstant.LONG_ZERO;
    ...
    static <T> TripleFunctionObjectLongIntToLong<T> empty() {
        return (TripleFunctionObjectLongIntToLong<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectLongIntToLong<T> constant(long value) {
     ...
     */
    long apply(T v1, long v2, int v3);
}
