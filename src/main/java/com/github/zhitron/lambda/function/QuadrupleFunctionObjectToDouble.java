 ...
 */
@FunctionalInterface
public interface QuadrupleFunctionObjectToDouble<T, U, V, O> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.DOUBLE_ZERO 值。
     */
    QuadrupleFunctionObjectToDouble<?, ?, ?, ?> EMPTY = (v1, v2, v3, v4) -> BasicConstant.DOUBLE_ZERO;
    ...
    static <T, U, V, O> QuadrupleFunctionObjectToDouble<T, U, V, O> empty() {
        return (QuadrupleFunctionObjectToDouble<T, U, V, O>) EMPTY;
    }
     ...
     */
    static <T, U, V, O> QuadrupleFunctionObjectToDouble<T, U, V, O> constant(double value) {
     ...
     */
    double apply(T v1, U v2, V v3, O v4);
}
