 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectTwDoubleToLong<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.LONG_ZERO 值。
     */
    TripleFunctionObjectTwDoubleToLong<?> EMPTY = (v1, v2, v3) -> BasicConstant.LONG_ZERO;
    ...
    static <T> TripleFunctionObjectTwDoubleToLong<T> empty() {
        return (TripleFunctionObjectTwDoubleToLong<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectTwDoubleToLong<T> constant(long value) {
     ...
     */
    long apply(T v1, double v2, double v3);
}
