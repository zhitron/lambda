 ...
 */
@FunctionalInterface
public interface SingleFunctionObjectToFloat<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.FLOAT_ZERO 值。
     */
    SingleFunctionObjectToFloat<?> EMPTY = (v1) -> BasicConstant.FLOAT_ZERO;
    ...
    static <T> SingleFunctionObjectToFloat<T> empty() {
        return (SingleFunctionObjectToFloat<T>) EMPTY;
    }
     ...
     */
    static <T> SingleFunctionObjectToFloat<T> constant(float value) {
        return (v1) -> value;
    }
