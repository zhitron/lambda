 ...
 */
@FunctionalInterface
public interface QuadrupleFunctionTriObjectIntToInt<T, U, V> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.INT_ZERO 值。
     */
    QuadrupleFunctionTriObjectIntToInt<?, ?, ?> EMPTY = (v1, v2, v3, v4) -> BasicConstant.INT_ZERO;
    ...
    static <T, U, V> QuadrupleFunctionTriObjectIntToInt<T, U, V> empty() {
        return (QuadrupleFunctionTriObjectIntToInt<T, U, V>) EMPTY;
    }
     ...
     */
    static <T, U, V> QuadrupleFunctionTriObjectIntToInt<T, U, V> constant(int value) {
     ...
     */
    int apply(T v1, U v2, V v3, int v4);
}
