 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectIntDoubleToByte<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.BYTE_ZERO 值。
     */
    TripleFunctionObjectIntDoubleToByte<?> EMPTY = (v1, v2, v3) -> BasicConstant.BYTE_ZERO;
    ...
    static <T> TripleFunctionObjectIntDoubleToByte<T> empty() {
        return (TripleFunctionObjectIntDoubleToByte<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectIntDoubleToByte<T> constant(byte value) {
     ...
     */
    byte apply(T v1, int v2, double v3);
}
