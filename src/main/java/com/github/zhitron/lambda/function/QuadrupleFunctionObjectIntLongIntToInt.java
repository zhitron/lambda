 ...
 */
@FunctionalInterface
public interface QuadrupleFunctionObjectIntLongIntToInt<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.INT_ZERO 值。
     */
    QuadrupleFunctionObjectIntLongIntToInt<?> EMPTY = (v1, v2, v3, v4) -> BasicConstant.INT_ZERO;
    ...
    static <T> QuadrupleFunctionObjectIntLongIntToInt<T> empty() {
        return (QuadrupleFunctionObjectIntLongIntToInt<T>) EMPTY;
    }
     ...
     */
    static <T> QuadrupleFunctionObjectIntLongIntToInt<T> constant(int value) {
     ...
     */
    int apply(T v1, int v2, long v3, int v4);
}
