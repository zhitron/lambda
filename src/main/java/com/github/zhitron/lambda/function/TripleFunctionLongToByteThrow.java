 ...
 * 该接口扩展自 {@link TripleFunctionLongToByte}，增加了异常处理能力。
 *
 * @param <E> 异常类型，必须是 {@link Exception} 的子类
 * @author zhitron
 */
@FunctionalInterface
public interface TripleFunctionLongToByteThrow<E extends Exception> extends TripleFunctionLongToByte {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.BYTE_ZERO 值。
     */
    TripleFunctionLongToByteThrow<?> EMPTY = (v1, v2, v3) -> BasicConstant.BYTE_ZERO;
    ...
    static <E extends Exception> TripleFunctionLongToByteThrow<E> empty() {
        return (TripleFunctionLongToByteThrow<E>) EMPTY;
    }
     ...
     */
    static <E extends Exception> TripleFunctionLongToByteThrow<E> constant(byte value) {
     ...
     * @throws E 抛出执行过程中的异常
     */
    byte applyThrow(long v1, long v2, long v3) throws E;
     ...
     */
    @Override
    default byte apply(long v1, long v2, long v3) {
        try {
