 ...
 */
@FunctionalInterface
public interface QuadrupleFunctionObjectToFloat<T, U, V, O> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.FLOAT_ZERO 值。
     */
    QuadrupleFunctionObjectToFloat<?, ?, ?, ?> EMPTY = (v1, v2, v3, v4) -> BasicConstant.FLOAT_ZERO;
    ...
    static <T, U, V, O> QuadrupleFunctionObjectToFloat<T, U, V, O> empty() {
        return (QuadrupleFunctionObjectToFloat<T, U, V, O>) EMPTY;
    }
     ...
     */
    static <T, U, V, O> QuadrupleFunctionObjectToFloat<T, U, V, O> constant(float value) {
     ...
     */
    float apply(T v1, U v2, V v3, O v4);
}
