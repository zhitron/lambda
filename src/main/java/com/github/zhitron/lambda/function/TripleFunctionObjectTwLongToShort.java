 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectTwLongToShort<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.SHORT_ZERO 值。
     */
    TripleFunctionObjectTwLongToShort<?> EMPTY = (v1, v2, v3) -> BasicConstant.SHORT_ZERO;
    ...
    static <T> TripleFunctionObjectTwLongToShort<T> empty() {
        return (TripleFunctionObjectTwLongToShort<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectTwLongToShort<T> constant(short value) {
     ...
     */
    short apply(T v1, long v2, long v3);
}
