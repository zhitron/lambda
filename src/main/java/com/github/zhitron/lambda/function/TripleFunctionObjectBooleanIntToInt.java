 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectBooleanIntToInt<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.INT_ZERO 值。
     */
    TripleFunctionObjectBooleanIntToInt<?> EMPTY = (v1, v2, v3) -> BasicConstant.INT_ZERO;
    ...
    static <T> TripleFunctionObjectBooleanIntToInt<T> empty() {
        return (TripleFunctionObjectBooleanIntToInt<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectBooleanIntToInt<T> constant(int value) {
     ...
     */
    int apply(T v1, boolean v2, int v3);
}
