 ...
 * 该接口扩展自 {@link TripleFunctionIntToLong}，增加了异常处理能力。
 *
 * @param <E> 异常类型，必须是 {@link Exception} 的子类
 * @author zhitron
 */
@FunctionalInterface
public interface TripleFunctionIntToLongThrow<E extends Exception> extends TripleFunctionIntToLong {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.LONG_ZERO 值。
     */
    TripleFunctionIntToLongThrow<?> EMPTY = (v1, v2, v3) -> BasicConstant.LONG_ZERO;
    ...
    static <E extends Exception> TripleFunctionIntToLongThrow<E> empty() {
        return (TripleFunctionIntToLongThrow<E>) EMPTY;
    }
     ...
     */
    static <E extends Exception> TripleFunctionIntToLongThrow<E> constant(long value) {
     ...
     * @throws E 抛出执行过程中的异常
     */
    long applyThrow(int v1, int v2, int v3) throws E;
     ...
     */
    @Override
    default long apply(int v1, int v2, int v3) {
        try {
