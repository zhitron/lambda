 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectLongIntToChar<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.CHAR_ZERO 值。
     */
    TripleFunctionObjectLongIntToChar<?> EMPTY = (v1, v2, v3) -> BasicConstant.CHAR_ZERO;
    ...
    static <T> TripleFunctionObjectLongIntToChar<T> empty() {
        return (TripleFunctionObjectLongIntToChar<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectLongIntToChar<T> constant(char value) {
     ...
     */
    char apply(T v1, long v2, int v3);
}
