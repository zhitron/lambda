 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectIntDoubleToInt<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.INT_ZERO 值。
     */
    TripleFunctionObjectIntDoubleToInt<?> EMPTY = (v1, v2, v3) -> BasicConstant.INT_ZERO;
    ...
    static <T> TripleFunctionObjectIntDoubleToInt<T> empty() {
        return (TripleFunctionObjectIntDoubleToInt<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectIntDoubleToInt<T> constant(int value) {
     ...
     */
    int apply(T v1, int v2, double v3);
}
