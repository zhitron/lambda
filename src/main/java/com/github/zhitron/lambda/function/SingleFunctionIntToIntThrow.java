 ...
 * 该接口扩展自 {@link SingleFunctionIntToInt}，增加了异常处理能力。
 *
 * @param <E> 异常类型，必须是 {@link Exception} 的子类
 * @author zhitron
 */
@FunctionalInterface
public interface SingleFunctionIntToIntThrow<E extends Exception> extends SingleFunctionIntToInt {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.INT_ZERO 值。
     */
    SingleFunctionIntToIntThrow<?> EMPTY = (v1) -> BasicConstant.INT_ZERO;
    ...
    static <E extends Exception> SingleFunctionIntToIntThrow<E> empty() {
        return (SingleFunctionIntToIntThrow<E>) EMPTY;
    }
     ...
     */
    static <E extends Exception> SingleFunctionIntToIntThrow<E> constant(int value) {
        return (v1) -> value;
    }
