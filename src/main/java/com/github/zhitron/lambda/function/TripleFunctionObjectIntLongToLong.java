 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectIntLongToLong<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.LONG_ZERO 值。
     */
    TripleFunctionObjectIntLongToLong<?> EMPTY = (v1, v2, v3) -> BasicConstant.LONG_ZERO;
    ...
    static <T> TripleFunctionObjectIntLongToLong<T> empty() {
        return (TripleFunctionObjectIntLongToLong<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectIntLongToLong<T> constant(long value) {
     ...
     */
    long apply(T v1, int v2, long v3);
}
