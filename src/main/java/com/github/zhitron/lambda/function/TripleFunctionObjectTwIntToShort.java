 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectTwIntToShort<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.SHORT_ZERO 值。
     */
    TripleFunctionObjectTwIntToShort<?> EMPTY = (v1, v2, v3) -> BasicConstant.SHORT_ZERO;
    ...
    static <T> TripleFunctionObjectTwIntToShort<T> empty() {
        return (TripleFunctionObjectTwIntToShort<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectTwIntToShort<T> constant(short value) {
     ...
     */
    short apply(T v1, int v2, int v3);
}
