 ...
 */
@FunctionalInterface
public interface TripleFunctionObjectIntBooleanToInt<T> {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.INT_ZERO 值。
     */
    TripleFunctionObjectIntBooleanToInt<?> EMPTY = (v1, v2, v3) -> BasicConstant.INT_ZERO;
    ...
    static <T> TripleFunctionObjectIntBooleanToInt<T> empty() {
        return (TripleFunctionObjectIntBooleanToInt<T>) EMPTY;
    }
     ...
     */
    static <T> TripleFunctionObjectIntBooleanToInt<T> constant(int value) {
     ...
     */
    int apply(T v1, int v2, boolean v3);
}
