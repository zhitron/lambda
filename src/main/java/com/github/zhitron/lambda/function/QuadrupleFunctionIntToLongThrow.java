 ...
 * 该接口扩展自 {@link QuadrupleFunctionIntToLong}，增加了异常处理能力。
 *
 * @param <E> 异常类型，必须是 {@link Exception} 的子类
 * @author zhitron
 */
@FunctionalInterface
public interface QuadrupleFunctionIntToLongThrow<E extends Exception> extends QuadrupleFunctionIntToLong {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.LONG_ZERO 值。
     */
    QuadrupleFunctionIntToLongThrow<?> EMPTY = (v1, v2, v3, v4) -> BasicConstant.LONG_ZERO;
    ...
    static <E extends Exception> QuadrupleFunctionIntToLongThrow<E> empty() {
        return (QuadrupleFunctionIntToLongThrow<E>) EMPTY;
    }
     ...
     */
    static <E extends Exception> QuadrupleFunctionIntToLongThrow<E> constant(long value) {
     ...
     * @throws E 抛出执行过程中的异常
     */
    long applyThrow(int v1, int v2, int v3, int v4) throws E;
     ...
     */
    @Override
    default long apply(int v1, int v2, int v3, int v4) {
        try {
