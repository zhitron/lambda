...
/**
 * 这是一个通用的 lambda 函数接口，用于对 2 个 float 类型参数进行操作。。
 *
 * @author zhitron
 */
@FunctionalInterface
public interface FloatTwiceOperator {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.FLOAT_ZERO 值。
     */
    FloatTwiceOperator EMPTY = (v1, v2) -> BasicConstant.FLOAT_ZERO;
     ...
     */
    static FloatTwiceOperator empty() {
        return EMPTY;
    }
     ...
     */
    static FloatTwiceOperator constant(float value) {
        return (v1, v2) -> value;
    }
