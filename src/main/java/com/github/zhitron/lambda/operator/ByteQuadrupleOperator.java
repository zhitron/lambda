...
/**
 * 这是一个通用的 lambda 函数接口，用于对 4 个 byte 类型参数进行操作。。
 *
 * @author zhitron
 */
@FunctionalInterface
public interface ByteQuadrupleOperator {

    /**
     * 一个空实现的实例，它总是返回 BasicConstant.BYTE_ZERO 值。
     */
    ByteQuadrupleOperator EMPTY = (v1, v2, v3, v4) -> BasicConstant.BYTE_ZERO;
     ...
     */
    static ByteQuadrupleOperator empty() {
        return EMPTY;
    }
     ...
     */
    static ByteQuadrupleOperator constant(byte value) {
     ...
     * @return 操作后的 byte 结果。
     */
    byte apply(byte v1, byte v2, byte v3, byte v4);
}
