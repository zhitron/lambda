...
/**
 * 表示一个接收三个 double 参数的谓词（布尔值函数），该谓词可能抛出异常。
 ...
 */
@FunctionalInterface
public interface DoubleTriplePredicateThrow<E extends Exception> extends DoubleTriplePredicate {
    /**
     * 在给定参数上评估此谓词，可能抛出异常。
     *
     * @param input 第一个输入参数
     ...
     */
    boolean testThrow(double input, double value, double other) throws E;
     ...
     */
    @Override
    default boolean test(double input, double value, double other) {
        try {
