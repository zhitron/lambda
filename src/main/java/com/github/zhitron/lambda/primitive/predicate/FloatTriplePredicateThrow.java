...
/**
 * 表示一个接收三个 float 参数的谓词（布尔值函数），该谓词可能抛出异常。
 ...
 */
@FunctionalInterface
public interface FloatTriplePredicateThrow<E extends Exception> extends FloatTriplePredicate {
    /**
     * 在给定参数上评估此谓词，可能抛出异常。
     *
     * @param input 第一个输入参数
     ...
     */
    boolean testThrow(float input, float value, float other) throws E;
     ...
     */
    @Override
    default boolean test(float input, float value, float other) {
        try {
