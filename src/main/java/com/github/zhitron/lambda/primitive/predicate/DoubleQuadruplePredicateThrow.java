...
/**
 * 表示一个接收四个 double 参数的谓词（布尔值函数），该谓词可能抛出异常。
 ...
 */
@FunctionalInterface
public interface DoubleQuadruplePredicateThrow<E extends Exception> extends DoubleQuadruplePredicate {
    /**
     * 在给定参数上评估此谓词，可能抛出异常。
     *
     * @param input   第一个输入参数
     ...
     */
    boolean testThrow(double input, double value, double other, double another) throws E;
     ...
     */
    @Override
    default boolean test(double input, double value, double other, double another) {
        try {
