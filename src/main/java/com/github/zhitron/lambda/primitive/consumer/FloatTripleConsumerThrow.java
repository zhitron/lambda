...
/**
 * 表示一个接收三个 float 参数的消费器，该消费器可能抛出异常。
 ...
 */
@FunctionalInterface
public interface FloatTripleConsumerThrow<E extends Exception> extends FloatTripleConsumer {
    /**
     * 在给定参数上执行消费操作，可能抛出异常。
     *
     * @param input 第一个输入参数
     ...
     */
    void acceptThrow(float input, float value, float other) throws E;
     ...
     */
    @Override
    default void accept(float input, float value, float other) {
        try {
