...
/**
 * 表示一个接收单个 double 参数的消费器，该消费器可能抛出异常。
 ...
 */
@FunctionalInterface
public interface DoubleSingleConsumerThrow<E extends Exception> extends DoubleSingleConsumer {
    /**
     * 在给定参数上执行消费操作，可能抛出异常。
     *
     * @param input 输入参数
     ...
     */
    void acceptThrow(double input) throws E;
     ...
     */
    @Override
    default void accept(double input) {
        try {
            acceptThrow(input);
        } catch (Exception e) {
